...
bool IsVisible(const Vector_t& start, const Vector_t& end, const C_CSPlayerPawn* target) {
    ...
    for (int i = 0; i < bone_count; ++i) {
        ...
        QAngle_t view_angles = *reinterpret_cast<QAngle_t*>(modules.getModule("client") + 0x1A75620); // dwViewAngles
...
void Aimbot() {
    ...
    QAngle_t* view_angles = reinterpret_cast<QAngle_t*>(modules.getModule("client") + 0x1A75620);
