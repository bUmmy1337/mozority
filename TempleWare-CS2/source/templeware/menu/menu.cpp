...
const char* fontPath = "C:/Mozority/Fonts/fa-solid-900.ttf";
const char* fontURL = "https://github.com/FortAwesome/font-awesome-sass/raw/refs/heads/main/assets/fonts/font-awesome/fa-solid-900.ttf";

void EnsureFontAwesomeDownloaded() {
    try {
        ...
        if (!std::filesystem::exists(dir)) {
            ...
            if (ec) {
                std::cerr << "[menu] Failed to create font directory: " << ec.message() << "\n";
                return;
            }
        }

        // Если файл уже есть — ок
        if (std::filesystem::exists(path)) return;

        std::cout << "[menu] Font not found, downloading from: " << fontURL << std::endl;

        HRESULT hr = URLDownloadToFileA(nullptr, fontURL, fontPath, 0, nullptr);
        if (SUCCEEDED(hr)) {
            std::cout << "[menu] Font downloaded successfully.\n";
        }
        else {
            std::cerr << "[menu] Failed to download font. HRESULT: 0x" << std::hex << hr << std::dec << std::endl;
        }
    }
    catch (const std::exception& e) {
        std::cerr << "[menu] Exception in EnsureFontAwesomeDownloaded: " << e.what() << "\n";
    }
    catch (...) {
        std::cerr << "[menu] Unknown crash in EnsureFontAwesomeDownloaded()\n";
    }
}
...
void Menu::init(HWND& window, ID3D11Device* pDevice, ID3D11DeviceContext* pContext, ID3D11RenderTargetView* mainRenderTargetView) {
    ...
    EnsureFontAwesomeDownloaded();

    // Подождать 1 секунду после возможной загрузки файла

    // === Загрузка Arial первым ===
    ImFont* arialFont = io.Fonts->AddFontFromFileTTF("C:/Windows/Fonts/arial.ttf", 16.0f);
    smallFont = io.Fonts->AddFontFromFileTTF("C:/Windows/Fonts/arial.ttf", 12.0f);

    if (!arialFont) {
        std::cerr << "[menu] Failed to load Arial!\n";
    }

    // === Подключаем FontAwesome с merge ===
    ImFontConfig faConfig;
    faConfig.MergeMode = true;
    faConfig.PixelSnapH = true;

    static const ImWchar faRange[] = { 0xf000, 0xf8ff, 0 };
    ImFont* faFont = io.Fonts->AddFontFromFileTTF(fontPath, 14.0f, &faConfig, faRange);

    if (!faFont) {
        std::cerr << "[menu] Failed to load FontAwesome!\n";
    }

    // Построение итогового шрифта
    io.Fonts->Build();
    ...
    std::cout << "initialized menu with FontAwesome + Arial\n";
}
